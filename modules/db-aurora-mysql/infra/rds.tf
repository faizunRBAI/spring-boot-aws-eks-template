...
resource "aws_security_group" "db" {
  ...
  description = "Aurora MySQL access for the EKS workloads of ${local.name}"
  vpc_id      = aws_vpc.this.id

  ingress {
    description = "Aurora MySQL from the cluster node security group"
    from_port   = 3306
    to_port     = 3306
    protocol    = "tcp"
...
output "database_jdbc_url" {
  ...
  value       = "jdbc:mysql://${aws_rds_cluster.this.endpoint}:${aws_rds_cluster.this.port}/appdb?sslMode=VERIFY_IDENTITY"
}
