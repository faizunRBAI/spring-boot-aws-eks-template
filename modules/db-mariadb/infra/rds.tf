...
resource "aws_security_group" "db" {
  ...
  description = "MariaDB access for the EKS workloads of ${local.name}"
  vpc_id      = aws_vpc.this.id

  ingress {
    description = "MariaDB from the cluster node security group"
    from_port   = 3306
    to_port     = 3306
    protocol    = "tcp"
...
output "database_jdbc_url" {
  ...
  value       = "jdbc:mariadb://${aws_db_instance.this.address}:${aws_db_instance.this.port}/appdb?sslMode=verify-full"
}
